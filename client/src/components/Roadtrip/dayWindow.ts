...
import type { RoadtripDay, RoadtripStop, RoutedLeg } from './useRoadtripRoutes'
...
export function dayWindow(start: unknown, end: unknown, endMode?: unknown): DayWindow | null {
  ...
  return from !== null && to !== null && to > from ? { start: from, end: to, ...(endMode === 'stop' ? { endMode } as const : {}) } : null
}
...
function portion(leg: RoutedLeg, from: number, to: number, unit: DistanceUnit): RoutedLeg {
  ...
  const line = metres === 0 ? coordinates : sliceAtMeters(spine, from * metres, to * metres).map(p => [p.lat, p.lng] as [number, number])
  ...
  return {
    seg: {
      ...leg.seg, distance, duration, distanceText, durationText,
      drivingText: `${distanceText} · ${durationText}`,
      walkingText: `${distanceText} · ${durationText}`,
      from: line[0], to: line[line.length - 1], mid: line[Math.floor(line.length / 2)],
    },
    line,
...
function stationary(stop: RoadtripStop): RoutedLeg {
  ...
  return {
    line: [], vias: [],
    seg: { from: at, to: at, mid: at, distance: 0, duration: 0, mode: 'driving', walkingText: '', drivingText: '', distanceText: '' },
  }
}
...
export function planDayWindow(
  days: PlanDay[], window: DayWindow, lookup: LegLookup, unit: DistanceUnit,
  ...
  if (legs.some(l => !l || !Number.isFinite(l.seg.duration) || l.seg.duration < 0 || l.line.length < 2 && l.seg.distance > 0)) {
  ...
  const chainAt = (n: number): SpillChain => {
    ...
    const chain: SpillChain = {
      ...
      dayNumber: n, date, title: stored?.title ?? null,
      stops: [], schedule: { entries: [], warnings: [] }, spills: [],
    }
    chains.set(n, chain)
    return chain
  }
  ...
  const append = (stop: RoadtripStop, arrival: number, departure = arrival) => {
    ...
    chain.schedule.entries.push({
      arrival: formatClock(Math.round(arrival)), departure: formatClock(Math.round(departure)),
  ...
  const night = (at: RoadtripStop, final = false): RoadtripStop => {
    ...
    const end: RoadtripStop = {
      ...at, assignmentId: -2000000000 - number * 2,
      name: labels.end, time: null, dwellMinutes: 0, stopType: null, fillPercent: null,
    ...
    const start: RoadtripStop = {
      ...end, assignmentId: end.assignmentId - 1, name: labels.start,
  ...
  for (let i = 0; i < stops.length; i++) {
    ...
    if (previous) {
      ...
      while (fraction < 1) {
        ...
        if (!manualCut && (target !== undefined || !split || clock + minutes * (1 - fraction) <= window.end + 0.000001)) break
  ...
  if ([...targets.keys()].some(day => !chains.get(day)?.stops.some(stop => stop.automaticNight?.phase === 'end'))) return failed('conflict')
  ...
  for (const chain of chains.values()) {
    for (let i = 0; i < chain.stops.length; i++) {
      ...
      else chain.spills.push({
        automatic: true,
        at: i, count: 1, fromDayNumber: owner.dayNumber, departure: null,
        leg: undefined, line: [], fromStop: undefined,
      })
    }
  }
...
export function roadtripInsertion(day: Pick<RoadtripDay, 'stops'>, index: number) {
  ...
  return stop ? { dayId: stop.ownerDayId, position: stop.ownerIndex + (stop.automaticNight || index >= day.stops.length ? 1 : 0) } : null
}
