...
import type { RoadtripStop, RoutedLeg } from './useRoadtripRoutes'
...
const stop = (id: number, patch: Partial<RoadtripStop> = {}): RoadtripStop => ({
  assignmentId: id, placeId: id, ownerDayId: 1, ownerIndex: id - 1,
  lat: 0, lng: id - 1, name: `Stop ${id}`, time: null, dwellMinutes: 0,
  legMode: null, incomingLegMode: null, stopType: null, ...patch,
})
const day = (number: number, stops: RoadtripStop[] = []) => ({
  dayId: number, dayNumber: number, date: `2026-09-${String(number + 10).padStart(2, '0')}`, title: null, stops,
})
const leg = (minutes: number, from: RoadtripStop, to: RoadtripStop): RoutedLeg => ({
  line: [[from.lat, from.lng], [to.lat, to.lng]], vias: [],
  seg: {
    from: [from.lat, from.lng], to: [to.lat, to.lng], mid: [0, (from.lng + to.lng) / 2],
    distance: minutes * 1000, duration: minutes * 60, mode: 'driving',
    distanceText: '', drivingText: '', walkingText: '',
  },
})
const calculate = (stops: RoadtripStop[], minutes: number[] = [720], extra = [day(2)]) =>
  planDayWindow([day(1, stops), ...extra], hours, (a, b) => leg(minutes[stops.indexOf(a)] ?? 60, a, b), 'metric', labels)
...
describe('daily travel window', () => {
  ...
  it.each([0.25, 0.9])('moves the cut in either direction and preserves the whole drive (%s)', fraction => {
    ...
    expect(manual.legFor(stops[0], end)!.seg.duration + manual.legFor(start, stops[1])!.seg.duration).toBeCloseTo(720 * 60)
  })

  it('does not pull a visit with a fixed time out of its stored day', () => {
    const pinned = stop(2, { ownerDayId: 2, time: '12:00' })
    const manual = planDayWindow([day(1, [stop(1)]), day(2, [pinned])], hours, (a, b) => leg(60, a, b), 'metric', labels,
      [{ day_number: 1, from_assignment_id: 2, to_assignment_id: null, fraction: 1 }])
  ...
  it('rejects crossed or disconnected manual boundaries', () => {
    ...
    for (const boundaries of [
      ...
      [{ day_number: 1, from_assignment_id: 3, to_assignment_id: null, fraction: 1 }, { day_number: 2, from_assignment_id: 2, to_assignment_id: null, fraction: 1 }],
    ]) expect(planDayWindow([day(1, stops)], hours, (a, b) => leg(60, a, b), 'metric', labels, boundaries).issue).toBe('conflict')
  })
  ...
  it('mixes an explicit stop end with automatic cuts on later drives', () => {
    ...
    expect(planned.chains.map(d => d.schedule.entries.map(e => e.arrival))).toEqual([
      ['08:00', '10:00', '11:00'], ['08:00', '18:00'], ['08:00', '10:00'],
    ])
  ...
  it('requires two valid increasing HH:mm values', () => {
    ...
    for (const pair of [[null, '18:00'], ['08:00', ''], ['8:00', '18:00'], ['24:00', '25:00'], ['18:00', '08:00'], ['08:00', '08:00'], [800, 1800]]) {
  ...
  it('splits the drive at day end and resumes at the same point next morning', () => {
    ...
    expect(planned.issue).toBeNull()
    const [today, tomorrow] = planned.chains
  ...
  it('reaches a last place exactly at the cutoff and still respects manual times', () => {
    ...
    const next = stop(2)
    const calculateAtStop = (minutes: number) => planDayWindow([day(1, [first, next])], { ...hours, endMode: 'stop' },
      (a, b) => leg(minutes, a, b), 'metric', labels)
  ...
  it('reports an indivisible drive that cannot fit rather than adding endless empty days', () => {
    const planned = planDayWindow([day(1, [stop(1), stop(2)])], { ...hours, endMode: 'stop' },
      (a, b) => leg(720, a, b), 'metric', labels)
